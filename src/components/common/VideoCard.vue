...
const cardStyle = computed(() => ({
  transform: isHovering.value 
    ? `perspective(1000px) rotateX(${rotateX.value}deg) rotateY(${rotateY.value}deg) scale(1.02) translateZ(0)`
    : 'perspective(1000px) rotateX(0deg) rotateY(0deg) scale(1) translateZ(0)',
  transition: isHovering.value ? 'transform 0.08s linear' : 'transform 0.25s ease-out'
}))

function updateTransform() {
  rotateX.value = pendingRotateX
  rotateY.value = pendingRotateY
  rafId = null
}
...
function handleMouseMove(e: MouseEvent) {
  ...
  pendingRotateX = Math.round((y - centerY) / centerY * -10)
  pendingRotateY = Math.round((x - centerX) / centerX * 10)
