...
- **圆角样式**: 使用 `border-radius: var(--radius-xl)` 实现圆角
- **尺寸调整**: 弹出搜索框最大宽度 500px
- **搜索建议**: 在搜索框下方显示匹配建议
- **输入框样式**: 底部横线样式，聚焦时变为主题色
...
## 22. 搜索框与登录页面修复 (2026-02-14)

### 22.1 搜索弹出框样式修复 ✅

- **修复药丸形状问题**: 将 `border-radius: var(--radius-full)` 改为 `var(--radius-xl)`
- **调整布局**: 搜索框输入区域使用底部横线样式
- **增加内边距**: 使搜索框更加清晰可见
- **搜索建议样式**: 优化下拉建议的间距和布局

### 22.2 登录页面修复 ✅

- **修复黑色主题背景色**: 移除渐变背景，使用 `var(--bg-color)` 统一背景
- **添加忘记密码/注册按钮**: 在登录按钮下方添加链接
- **修复错误提示位置**: 错误提示显示在输入框下方，而非右侧
- **输入框样式统一**: 使用与搜索页相同的横线样式
- **Logo 和标题可点击**: 点击网站图标和名称可返回主页

### 22.3 页面头部文字放大 ✅

- **Logo 文字**: 从 `var(--font-size-base)` 改为 `var(--font-size-lg)`
- **分区切换按钮**: 从 `var(--font-size-sm)` 改为 `var(--font-size-base)`

### 22.4 修改文件

| 文件路径 | 修改内容 |
|---------|---------|
| `src/components/layout/AppHeader.vue` | 修复搜索弹出框样式，放大头部文字 |
| `src/views/Login.vue` | 重构登录页面，修复所有问题 |
